...
-- Public contact-form submissions (C1, SECURITY.md SEC-23): every enquiry is
-- stored; the (ip, created_at) index powers the per-IP hourly rate limit.
...
-- Home-page student testimonials (C3). name_html is rich (sanitized) because
-- the shipped headings contain <br> line breaks.
CREATE TABLE IF NOT EXISTS testimonials (
  id          INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  name_html   VARCHAR(200) NOT NULL,
  body_html   MEDIUMTEXT   NOT NULL,
  bg_image_id INT UNSIGNED NULL,           -- N2: optional card background (NULL = shipped CSS static)
  position    SMALLINT UNSIGNED NOT NULL DEFAULT 0,
  is_active   TINYINT(1)   NOT NULL DEFAULT 1,
  created_at  DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at  DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  KEY idx_pos (position),
  CONSTRAINT fk_testi_bg FOREIGN KEY (bg_image_id) REFERENCES images(id) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
