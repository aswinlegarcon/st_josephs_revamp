...
**Revision (N1, 2026-08-15 — owner decision):** the lockout is visible again, deliberately. After exhausting the 5 attempts (or while locked, even with the correct password), `login.php` shows *"Too many failed sign-in attempts — this account is temporarily locked… wait about N minutes"*. Enumeration is masked within a session by a **shadow counter** (`$_SESSION['sj_lf']`): 5 failures for ANY username string — real or not — produce the same lock message, so the message cannot cheaply confirm an account. **Accepted residual:** a cross-session prober (new cookie jar, 1 attempt against a genuinely locked account) can still distinguish lock state; accepted because the owner-UX cost of a silent lock was real (this happened), usernames here are low-value, and the lock itself still blocks the brute force. Wrong-password vs unknown-user remain byte-identical; `sleep(1)` stays on every failure path.
**Agent verification:** script 20 wrong-password attempts → lockout persists past the first window; "wrong user" vs "wrong password" byte-identical below 5 tries; 5 failures on a FAKE username → same lock message as a real one (same session); correct password while locked → lock message, not a success or a distinct error.
...
**Regression + fix (N7-fix, 2026-08-15 — owner-reported):** the N7 dashboard polls `?r=stats` every 20 s, and every request refreshed `last_seen`. An open dashboard tab therefore renewed the session forever, so the 30-minute idle timeout could never fire and only the 12-hour absolute cap ended a session (the owner was still signed in after ~10 h). **Rule established: a request no human made must have its timeouts ENFORCED but must never count as ACTIVITY.** Endpoints that are background heartbeats define `SJ_PASSIVE_REQUEST` **before** including the bootstrap (`admin/api/stats.php`, `admin/health.php`); `Auth::boot()` then skips the `last_seen` update and the id rotation while still applying both timeouts. `dashboard.js` additionally pauses its interval while `document.hidden` (a background tab neither wakes the server nor looks alive), always polls once on load and on tab re-focus, and on a `401` shows "Signed out" and returns to the sign-in page. **Any future polling endpoint MUST declare `SJ_PASSIVE_REQUEST` — otherwise it silently disables the idle timeout.**
**Agent verification (N7-fix):** age a live session's `last_seen` beyond 1800 s, call `?r=stats` → the idle clock must be UNCHANGED and the call must 401 once past the limit; load any panel page → the clock resets to ~0 (real activity still counts); the panel page then 302s to login and the session file is destroyed.
...
### SEC-24 Password recovery endpoint — A07/A05
**Where:** `public_html/admin/recover.php` (web, unauthenticated by design) and `database/reset-admin-password.php` (CLI).
**Status: ✅ shipped guarded (N1, 2026-08-15).** Prod has no SSH and no reliable outbound mail, so "forgot password" is proven by **filesystem control**: the page is a 404 unless `config/recovery-token.txt` exists **above the webroot** with a ≥16-char token. Whoever can create that file (mPanel file manager/FTP) already owns the site, so the page grants no new authority. Controls: token compared with `hash_equals`; token+username failures collapse into ONE message with `sleep(1)` (no oracle during a window); CSRF-checked POST; new password ≥12; on success the hash is replaced, lockout cleared, `session_regenerate_id(true)`, the token file is **deleted (single use)** — with an on-screen red warning if `unlink` fails; `recover.ok`/`recover.fail`/`recover.cli` audited (never the token or password). The token file is gitignored; the CLI variant is `PHP_SAPI === 'cli'`-gated and prints a one-time temp password with `must_change_password = 1`.
**Residual risk:** while a window is armed, the token is brute-forceable in theory → the runbook (DEPLOY.md §9) mandates a ≥24-char random token and immediate use; the file's presence enables the login-page "Forgot password?" link, which is information an attacker could notice during a window — accepted (windows are minutes long, self-destruct on use).
**Agent verification:** GET/POST `/admin/recover.php` with no token file → 404; with a <16-char token file → 404; armed + wrong token → generic failure, `recover.fail` audited, 1s delay; armed + valid token + short password → rejected without consuming the file; full success → password works, lock cleared, file gone, second POST → 404.
